...
export default function Curso({navigation }){
    ...
    async function showConfirmDialog  ( )  {
        const message = 'Deseja excluir esse curso?';
        return Alert.alert(
            "Confirmação",
            message,
            [
    ...
    async function onyesdialog(){
        try{
            const response = await api.delete(`/cursos/deletecurso/${id}`);
    ...
    async function getcurso(){
        try{
            ...
            const response = await api.get(`/cursos/detail/${id}`)
            setCurso(response.data as CursoProps)
        }catch (err: AxiosError|any){
            Toast.show({
                type:'error',
                text1:'Erro',
                text2:(!!err.data.response.message)?err.data.response.message:'Erro ao obter dados do curso'
            })
        }
    ...
    async function handlesetinscricao(){
             ...
             await api.post(`/cursos/setinscricao/${id}`).then((response)=>{
                 console.log(response.data)
    ...
    async function handleEditarcurso(){
    ...
    async function handleunsetinscricao(){
        ...
        await api.delete(`/cursos/unsetinscricao/${id}`).then((response)=>{
            console.log(response.data)
        }).catch((err:AxiosError)=>{
        ...
        <SafeAreaView style={containerstyle.container}>
            <ScrollView style={{width:'100%'}}>
                ...
                <Card mode="contained" style={styles.cardcontainer}>
                    <Card.Title titleStyle={{color:theme.colors.font}} title="Curso"  left={LeftContent} />
                    <Text style={{fontSize:25,marginVertical:10,color:theme.colors.font}}>{curso.name}</Text>
                    ...
                    {!!curso.local?<Text variant="bodyMedium" style={{fontSize:15,marginVertical:10,textAlign:'center',color:theme.colors.font}}>Local: {curso.local}</Text>:''}
                    {!!curso.professor?<Text variant="bodyMedium" style={{fontSize:15,marginVertical:10,textAlign:'center',color:theme.colors.font}}>Professor: {curso.professor}</Text>:''}
                    {!!curso.vagas?<Text variant="bodyMedium" style={{fontSize:15,marginVertical:10,textAlign:'center',color:theme.colors.font}}>Vagas: {curso.vagas}</Text>:''}

                    {!!curso.vagas?<Text variant="bodyMedium" style={{fontSize:15,marginVertical:10,textAlign:'center',color:theme.colors.font}}>Restantes: {parseInt(curso.vagas)   - listainscritos.length}</Text>:''}
                    ...
                    {isAdmin?
                        <View>
                            <Button onPress={handleEditarcurso}  style={{marginVertical:5}} icon="file-edit" mode="contained">Editar</Button>
                            <Button onPress={()=>showConfirmDialog()} style={{marginVertical:5}} icon="trash-can" mode="contained">Excluir curso</Button>
                        </View>
                        :''
                    }
                    <Text variant="bodyMedium" style={{fontSize:15,marginVertical:10,textAlign:'center',color:theme.colors.font}}>{curso.descricao}</Text>
                </Card>
                </View>
                <Portal>
